...
export class SetlistFmClient {
  ...
  /** Serialize requests so consecutive calls start at least minRequestGapMs apart. */
  private throttled<T>(task: () => Promise<T>): Promise<T> {
    const run = this.queue.then(async () => {
      const wait = this.lastRequestStart + this.minRequestGapMs - this.nowFn();
      if (wait > 0) await this.sleepFn(wait);
      this.lastRequestStart = this.nowFn();
      return task();
    });
    this.queue = run.catch(() => {}); // a failed request must not poison the queue
    return run;
  }
